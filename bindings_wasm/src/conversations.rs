...
use crate::utils::to_value_with_bigint;
...
impl WasmConversations {
  ...
  #[wasm_bindgen]
  pub async fn list(&self, opts: Option<WasmListConversationsOptions>) -> Result<JsValue, JsError> {
    let opts = match opts {
      Some(options) => options,
    ...
    let convo_list: Vec<WasmGroup> = self
      .inner_client
    ...
    Ok(to_value_with_bigint(&convo_list)?)
  }
}
